...
// Enable CORS for all origins (needed for phone access)
app.use(cors());
        ...
        <script>
          ...
          function generateInvoice() {
            window.parent.postMessage({
              type: 'tool',
              payload: { toolName: 'generate_invoice', params: { amount: selectedAmount } }
            }, '*');
          }
          ...
          function checkPayment() {
            window.parent.postMessage({
              type: 'tool',
              payload: { toolName: 'check_payment', params: {} }
            }, '*');
          }
        </script>
      </body>
      ...
      const html = `
        ...
        <head>
          ...
          <style>
            ...
            .invoice-label { font-size: 0.85rem; color: #8892a0; }
            .invoice-amount { font-size: 1.25rem; font-weight: 700; color: #F7931A; }
            .qr-container {
              background: white;
              border-radius: 12px;
              padding: 16px;
            ...
            .qr-image { width: 200px; height: 200px; margin: 0 auto; display: block; }
            .bolt11-container {
            ...
            .timer { text-align: center; color: #6b7280; font-size: 0.85rem; margin-top: 12px; }
          </style>
        </head>
        <body>
          <div class="container">
            ...
            <div class="button-group">
              <button class="btn btn-primary" onclick="copyInvoice()">Copy Invoice</button>
          ...
          <script>
            ...
            async function copyInvoice() {
              try {
                ...
                window.parent.postMessage({ type: 'notify', payload: { message: 'Invoice copied to clipboard!' } }, '*');
              } catch (err) {
                window.parent.postMessage({ type: 'notify', payload: { message: 'Failed to copy invoice' } }, '*');
              }
            }

            function checkStatus() {
              window.parent.postMessage({ type: 'tool', payload: { toolName: 'check_payment', params: {} } }, '*');
            }
          </script>
        </body>
        </html>
  ...
  server.tool('check_payment', 'Checks the payment status of the current Lightning invoice', {}, async () => {
    ...
    const paidHtml = `
      ...
      <head>
        ...
        <style>
          ${commonStyles}
          .success-icon { width: 100px; height: 100px; margin: 0 auto 24px; display: block; }
          .success-message { text-align: center; margin-bottom: 32px; }
          .success-message h2 { color: #22c55e; font-size: 1.5rem; margin-bottom: 8px; }
          .success-message p { color: #8892a0; }
          .details-card {
          ...
          .detail-label { color: #8892a0; font-size: 0.9rem; }
      ...
      <body>
        ...
        <div class="container">
          ...
          <button class="btn btn-primary" onclick="newTip()">Send Another Tip</button>
        </div>
        <script>
          ...
          function newTip() {
            window.parent.postMessage({ type: 'tool', payload: { toolName: 'show_tip_jar', params: {} } }, '*');
          }
        </script>
      </body>
      </html>
    ...
    const pendingHtml = `
      ...
      <head>
        ...
        <style>
          ${commonStyles}
          .pending-icon {
            width: 100px; height: 100px; margin: 0 auto 24px; display: block;
            animation: spin 2s linear infinite;
          }
          @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
          .pending-message { text-align: center; margin-bottom: 32px; }
          .pending-message h2 { color: #fbbf24; font-size: 1.5rem; margin-bottom: 8px; }
          .pending-message p { color: #8892a0; }
          .status-card {
          ...
          .status-text { color: #fbbf24; font-weight: 600; margin-bottom: 8px; }
          .status-subtext { color: #8892a0; font-size: 0.85rem; }
      ...
      <body>
        <div class="container">
          ...
          <div class="button-group">
            ...
            <button class="btn btn-secondary" onclick="backToTipJar()">Back to Tip Jar</button>
          </div>
        </div>
        <script>
          function checkAgain() {
...
app.get('/health', (_req: Request, res: Response) => {
  res.json({ status: 'ok', server: 'bitcoin-builder-ui', version: '1.0.0' });
});
